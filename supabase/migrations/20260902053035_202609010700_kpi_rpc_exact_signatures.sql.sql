-- Monotoli KPI RPC v3 — EXACT MATCH con actions/kpi.ts
-- DROPs exhaustivos de TODAS las combinaciones previas (de 0400 y 0500) para limpiar firmas.
...
-- 1) kpi_sprint_burndown (p_sprint_id = nuevo nombre para MATCH frontend)
...
-- 2) kpi_velocity_summary: p_board_id, p_last_n_sprints
...
-- 3) kpi_assigned_sp_by_member: solo 2 params (tenant, sprint_id). Campos assigned_sp, completed_sp.
...
-- 4) kpi_sp_completed_over_time: 4 params, SIN p_board_id. bucket -> text field.
...
-- 5) kpi_cloud_costs_pivot: campos renombrados cost_usd/cost_mxn + bucket text
...
-- 6) kpi_daily_annotations_for_sprint: p_team_id, p_sprint_id. Fields: team_name, full_name, submission_id, answers jsonb.
...
grant execute on function public.kpi_assigned_sp_by_member(uuid, integer) to authenticated, service_role;
grant execute on function public.kpi_sp_completed_over_time(uuid, text, date, date) to authenticated, service_role;
